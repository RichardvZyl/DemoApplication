...
public sealed class UserRepository : EFRepository<User>, IUserRepository
{
    ...
    public async Task<UserModel> GetByAuthIdAsync(Guid id) => await Queryable.Where(UserExpression.AuthId(id)).Select(UserExpression.Model).SingleOrDefaultAsync();

    public async Task<UserModel> GetByIdAsync(Guid id) => await Queryable.Where(UserExpression.Id(id)).Select(UserExpression.Model).SingleOrDefaultAsync();

    public async Task<string> GetEmailById(Guid id) => await Queryable.Where(UserExpression.Id(id)).Select(UserExpression.Email()).SingleOrDefaultAsync();

    public async Task<string> GetFullNameByUserId(Guid id) => await Queryable.Where(UserExpression.Id(id)).Select(UserExpression.FullName()).SingleOrDefaultAsync();
