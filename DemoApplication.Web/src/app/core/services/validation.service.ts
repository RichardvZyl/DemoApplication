...
export class AppValidationService {
    ...
    isMax(value: any, max: string): boolean { return (!value || !max) || (parseFloat(value) <= parseFloat(max)); }

    isMin(value: any, min: string): boolean { return (!value || !min) || (parseFloat(value) >= parseFloat(min)); }
